        ...
        <table className={`${s.table} ${own.table}`}>
          ...
          <tbody>
            ...
            <tr className={s.detailRow}>
              ...
              {data.map((d) => <td key={d.year} className={s.tdValue}>{f(d.termLoan)}</td>)}
            </tr>
            <tr className={s.detailRow}>
              <td className={s.tdParticulars}>
                <span style={{ marginLeft: '40px', display: 'inline-block' }}>B) Unsecured Loans (Market/External)</span>
              </td>
              {data.map((d) => <td key={d.year} className={s.tdValue}>{f(d.unsecured)}</td>)}
            </tr>
            ...
            <tr className={s.detailRow}>
              ...
              {data.map((d) => <td key={d.year} className={s.tdValue}>{f(d.bankBorrowings)}</td>)}
            </tr>
            <tr className={s.detailRow}>
              <td className={s.tdParticulars}>
            ...
            <tr className={s.detailRow}>
              ...
              {data.map((d) => <td key={d.year} className={s.tdValue}>{f(d.otherCL)}</td>)}
            </tr>

            <tr className={s.grandTotalRow}>
              <td className={s.tdParticulars}>Total Sources of Funds</td>
              {data.map((d) => <td key={d.year} className={s.tdValue}>{f(d.totalLiab)}</td>)}
            </tr>
            ...
            <tr className={s.detailRow}>
              ...
              {data.map((d) => <td key={d.year} className={s.tdValue}>{f(d.grossFA)}</td>)}
            </tr>
            <tr className={s.detailRow}>
              <td className={s.tdParticulars}>
                <span style={{ marginLeft: '40px', display: 'inline-block' }}>B) Less: Accumulated Depreciation</span>
              </td>
              {data.map((d) => <td key={d.year} className={s.tdValue}>({f(d.accDepn)})</td>)}
            </tr>
            <tr className={s.subtotalRow}>
              <td className={s.tdParticulars}>
            ...
            <tr className={s.detailRow}>
              ...
              {data.map((d) => <td key={d.year} className={s.tdValue}>{f(d.inventory)}</td>)}
            </tr>
            <tr className={s.detailRow}>
              <td className={s.tdParticulars}>
            ...
            {data.some((d) => d.reconAdj > 0) && (
              <tr className={s.detailRow}>
                ...
                {data.map((d) => <td key={d.year} className={s.tdValue}>{f(d.reconAdj)}</td>)}
              </tr>
            )}

            <tr className={s.grandTotalRow}>
              <td className={s.tdParticulars}>Total Application of Funds</td>
              {data.map((d) => <td key={d.year} className={s.tdValue}>{f(d.totalAssets)}</td>)}
            </tr>
          </tbody>
        </table>
      </div>
