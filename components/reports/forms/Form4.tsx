      ...
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-slate-200 text-[11px] whitespace-nowrap font-mono">
          ...
          <tbody className="divide-y divide-[#f0ede6] bg-white text-slate-900">
            ...
            {/* 1. Raw Materials with Sub-entries */}
            <tr>
              <td className="px-6 py-2 pl-6 font-sans font-bold">1. Raw Materials (incl. stores and other items used in manufacturing)</td>
            ...
            <tr className="italic text-slate-500 bg-slate-50/50 border-b border-slate-100">
            ...
            {/* 5. Receivables with Sub-entries */}
            <tr>
            ...
            <tr className="italic text-slate-500 bg-slate-50/50">
              <td className="px-6 py-1.5 pl-12 font-sans">a. Up to 6 months</td>
              {data.map(d => <td key={d.year} className="px-6 py-1.5 text-right">{fmt(d.debtors)}</td>)}
            </tr>
            <tr className="italic text-slate-500 bg-slate-50/50 border-b border-slate-100">
              <td className="px-6 py-1.5 pl-12 font-sans">b. More than 6 months</td>
              {data.map(d => <td key={d.year} className="px-6 py-1.5 text-right">0</td>)}
            </tr>
            <tr className="bg-blue-50/30 text-[10px] text-blue-600 italic">
            ...
            <tr><td className="px-6 py-2 pl-6 font-sans">11. Statutory liabilities (GST/PF/TDS)</td>{data.map(d => <td key={d.year} className="px-6 py-2 text-right">{fmt(d.otherCL * 0.4)}</td>)}</tr>
            <tr><td className="px-6 py-2 pl-6 font-sans">12. Other current liabilities & provisions</td>{data.map(d => <td key={d.year} className="px-6 py-2 text-right">{fmt(d.otherCL * 0.6)}</td>)}</tr>
            ...
            <tr className="bg-slate-50 font-bold border-y border-slate-200">
              ...
              {data.map(d => <td key={d.year} className="px-6 py-3 text-right font-mono text-slate-600">{fmt(d.totalCA - (d.totalCL + (d.totalCA - d.totalCL) * 0.75))}</td>)}
            </tr>
            <tr className="bg-slate-900 text-white font-bold">
              <td className="px-6 py-4 pl-6 font-sans uppercase tracking-widest text-xs">16. ASSESSED BANK FINANCE (14 - 15)</td>
              {data.map(d => <td key={d.year} className="px-6 py-4 text-right text-xl text-emerald-400 tracking-tighter">{fmt((d.totalCA - d.totalCL) * 0.75)}</td>)}
            </tr>
          </tbody>
        </table>
      </div>
